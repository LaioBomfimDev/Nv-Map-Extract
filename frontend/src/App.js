...
import { AlertTriangle, BarChart3, Search, FolderOpen, MessageCircle, Upload, Download, ChevronLeft, Map as MapIcon, Target, X } from './components/Icons';
...
const VALID_TABS = new Set(NAV.map(item => item.id));
...
export default function App() {
  const { hasUpdate, isBlocking, latest, extension } = useExtensionUpdates();
  ...
  const [dismissedUpdate, setDismissedUpdate] = useState(() => {
    try { return window.localStorage.getItem('fm-dismissed-update') || ''; } catch (_) { return ''; }
  });
  ...
  // A aba faz parte da URL para sobreviver a recargas e permitir links diretos.
  useEffect(() => {
    const url = new URL(window.location.href);
    if (tab === 'dashboard') url.searchParams.delete('tab');
    else url.searchParams.set('tab', tab);
    window.history.replaceState({ tab }, '', `${url.pathname}${url.search}${url.hash}`);
  }, [tab]);
  ...
  async function handleWizardImport(leads, metadata) {
    setUploading(true);
    setUploadMsg('');
    try {
      const res = await api.importLeads(metadata.keyword, metadata.city, leads, metadata);
      const d = res.data || {};
  ...
  const showUpdateBanner = hasUpdate && (isBlocking || dismissedUpdate !== latest?.version);

  function dismissUpdate() {
    if (isBlocking || !latest?.version) return;
    try { window.localStorage.setItem('fm-dismissed-update', latest.version); } catch (_) {}
    setDismissedUpdate(latest.version);
  }
    ...
    <div style={{ minHeight: '100vh', background: '#09090b', display: 'flex', flexDirection: 'column' }}>
      ...
      <header className="responsive-header" style={{ background: '#18181b', borderBottom: '1px solid #27272a', padding: '0 32px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <nav style={{ display: 'flex', gap: 4 }}>
          {NAV.map(n => (
            <button key={n.id} onClick={() => setTab(n.id)} className="nav-btn"
              ...
              <span>{n.label}</span>
              {n.id === 'updates' && hasUpdate && (
                <span aria-label="Nova versão disponível" title="Nova versão disponível"
                  style={{ minWidth: 18, height: 18, padding: '0 5px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: isBlocking ? '#ef4444' : '#10b981', color: '#fff', fontSize: 10, fontWeight: 800 }}>
                  {isBlocking ? '!' : '1'}
                </span>
              )}
            </button>
        ...
        <div className="upload-btn-container" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button
            ...
            {uploading ? <><Spinner /> Importando...</> : <><Upload size={14} /> Importar planilha</>}
          </button>
      ...
      {showUpdateBanner && (
        <div role={isBlocking ? 'alert' : 'status'}
          style={{ background: isBlocking ? 'rgba(239,68,68,.13)' : 'rgba(16,185,129,.12)', borderBottom: `1px solid ${isBlocking ? 'rgba(239,68,68,.38)' : 'rgba(16,185,129,.35)'}`, color: isBlocking ? '#fecaca' : '#a7f3d0', padding: '10px 32px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          {isBlocking ? <AlertTriangle size={16} color="#ef4444" /> : <Download size={16} color="#10b981" />}
          <span style={{ fontSize: 13 }}>
            {isBlocking
              ? `Sua extensão v${extension.version || '?'} precisa ser atualizada para continuar minerando.`
              : `A extensão v${latest?.version || ''} está disponível.`}
          </span>
          <button type="button" onClick={() => setTab('updates')}
            style={{ background: isBlocking ? '#ef4444' : '#10b981', color: '#fff', border: 0, padding: '7px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
            Ver atualização
          </button>
          {!isBlocking && <button type="button" onClick={dismissUpdate} aria-label="Lembrar depois"
            style={{ background: 'transparent', color: '#a1a1aa', border: 0, padding: 6, cursor: 'pointer' }}><X size={15} /></button>}
        </div>
      )}
      ...
      <div className="responsive-padding" style={{ flex: 1, padding: '32px', maxWidth: 1400, margin: '0 auto', width: '100%' }}>
        ...
        <TabPanel active={tab === 'dashboard'}>
          <Dashboard onSelectSearch={handleSelectSearch} onGoTo={setTab} onImportCSV={() => setImportOpen(true)} />
        </TabPanel>

        {/* Scraper tab (instruções da extensão) */}
        <TabPanel active={tab === 'scraper'}><ScraperTab onGoTo={setTab} /></TabPanel>
      ...
      <ImportWizard open={importOpen} onClose={() => setImportOpen(false)} onImport={handleWizardImport} />
    </div>
  );
}
