-- ============================================================================
-- Friendly Miner — proveniencia, CRM e execucoes confiaveis
--
-- Migracao cumulativa e idempotente para instalacoes que ja executaram
-- schema.sql. Pode ser executada novamente com seguranca no SQL Editor.
--
-- Principais mudancas:
--   * search_results preserva a relacao N:N entre leads e importacoes;
--   * import_leads_v2 registra origem, contadores, metadados e idempotencia;
--   * criacao/avanco de campanhas passam a ser transacionais via RPC;
--   * lead_activities/lead_tasks formam a timeline e agenda do CRM;
--   * mining_jobs registra ACK, progresso, falhas e resultado da mineracao.
-- ============================================================================

create extension if not exists pgcrypto;

-- ── Evolucao das tabelas existentes ─────────────────────────────────────────
alter table public.searches add column if not exists input_count int not null default 0;
alter table public.searches add column if not exists inserted_count int not null default 0;
alter table public.searches add column if not exists merged_count int not null default 0;
alter table public.searches add column if not exists skipped_count int not null default 0;
alter table public.searches add column if not exists ignored_count int not null default 0;
alter table public.searches add column if not exists rejected_count int not null default 0;
alter table public.searches add column if not exists completed_at timestamptz;
alter table public.searches add column if not exists error_message text;
alter table public.searches add column if not exists idempotency_key text;
alter table public.searches add column if not exists metadata jsonb not null default '{}'::jsonb;
alter table public.searches alter column source set default 'extension';

alter table public.campaigns add column if not exists idempotency_key text;
alter table public.campaigns add column if not exists metadata jsonb not null default '{}'::jsonb;

-- A origem principal e apenas um atalho legado. Excluir uma importacao nao deve
-- mais apagar um lead que tambem apareceu em outras importacoes.
alter table public.results drop constraint if exists results_search_id_fkey;
do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.results'::regclass
      and conname = 'results_search_id_fkey'
  ) then
    alter table public.results
      add constraint results_search_id_fkey
      foreign key (search_id) references public.searches(id) on delete set null;
  end if;
end $$;

-- ── Proveniencia N:N de importacoes ─────────────────────────────────────────
...
create index if not exists idx_search_results_user_search
  on public.search_results (user_id, search_id, observed_at desc);
create index if not exists idx_search_results_result
  on public.search_results (result_id, observed_at desc);
create unique index if not exists uq_searches_user_idempotency
  on public.searches (user_id, idempotency_key)
  where idempotency_key is not null and idempotency_key <> '';
create unique index if not exists uq_campaigns_user_idempotency
  on public.campaigns (user_id, idempotency_key)
  where idempotency_key is not null and idempotency_key <> '';

-- Converte o vinculo legado em proveniencia sem duplicar registros.
insert into public.search_results
  (user_id, search_id, result_id, disposition, source_snapshot, observed_at)
select r.user_id, r.search_id, r.id, 'inserted', '{}'::jsonb,
       coalesce(s.created_at, r.created_at, now())
from public.results r
join public.searches s on s.id = r.search_id and s.user_id = r.user_id
where r.search_id is not null
on conflict (search_id, result_id) do nothing;

update public.searches s
set total_results = x.total,
    input_count = case when s.input_count = 0 then x.total else s.input_count end,
    inserted_count = case
      when s.inserted_count = 0 and s.merged_count = 0 then x.total
      else s.inserted_count
    end,
    completed_at = coalesce(s.completed_at, s.created_at)
from (
  select search_id, count(*)::int as total
  from public.search_results
  group by search_id
) x
where s.id = x.search_id;

-- ── Timeline, tarefas e execucoes de mineracao ──────────────────────────────
...
create index if not exists idx_lead_activities_result
  on public.lead_activities (user_id, result_id, occurred_at desc);
create unique index if not exists uq_lead_activities_user_idempotency
  on public.lead_activities (user_id, idempotency_key)
  where idempotency_key is not null and idempotency_key <> '';
create index if not exists idx_lead_tasks_due
  on public.lead_tasks (user_id, status, due_at);
create index if not exists idx_lead_tasks_result
  on public.lead_tasks (user_id, result_id, created_at desc);
create index if not exists idx_mining_jobs_user_created
  on public.mining_jobs (user_id, created_at desc);
create index if not exists idx_mining_jobs_active
  on public.mining_jobs (user_id, status, heartbeat_at desc);
...
declare t text;
begin
  foreach t in array array['search_results','lead_activities','lead_tasks','mining_jobs'] loop
...
-- ============================================================================
-- IMPORTACAO V2
-- A funcao antiga continua existindo como wrapper para a extensao instalada.
-- ============================================================================
...
begin
  ...
  v_source := case lower(coalesce(nullif(trim(p_source), ''), 'extension'))
    ...
    when 'extensao' then 'extension'
    when 'spreadsheet' then 'spreadsheet'
    when 'planilha' then 'spreadsheet'
  ...
  -- Serializa repeticoes da mesma requisicao para evitar corrida no indice.
  ...
  v_filename := coalesce(
    nullif(p_metadata->>'originalFilename', ''),
    trim(coalesce(nullif(p_keyword, ''), 'maps') || ' ' || coalesce(p_city, ''))
  );

  insert into public.searches
    (user_id, filename, keyword, city, total_results, status, source,
     input_count, idempotency_key, metadata)
  values
    (v_uid, v_filename, nullif(p_keyword,''), nullif(p_city,''), 0, 'processing',
     v_source, jsonb_array_length(p_leads), nullif(p_idempotency_key,''),
  ...
  loop
    ...
    v_ig := trim(coalesce(v_lead->>'instagram',''));
    v_fb := trim(coalesce(v_lead->>'facebook',''));
    v_in := trim(coalesce(v_lead->>'linkedin',''));
    v_tw := trim(coalesce(v_lead->>'twitter',''));
    ...
    order by case
      when v_place <> '' and r.place_id = v_place then 1
      when length(v_phone) > 3 and r.phone = v_phone then 2
      else 3 end
    limit 1
    for update;
    ...
    if found then
      if v_strategy = 'skip' then
        ...
        update public.results r set
          ...
          latitude = coalesce(v_lat, r.latitude),
          longitude = coalesce(v_lng, r.longitude)
      ...
      values
        (v_uid, v_search_id, v_result_id,
         case when v_strategy = 'skip' then 'skipped' else 'merged' end,
         coalesce(v_lead, '{}'::jsonb))
      on conflict (search_id, result_id) do update
        set observed_at = excluded.observed_at,
            source_snapshot = excluded.source_snapshot;
    else
      insert into public.results
        (user_id, search_id, name, address, phone, website, category, rating,
         reviews_count, latitude, longitude, place_id, cid, email, instagram,
         facebook, linkedin, twitter, youtube)
      values
        (v_uid, v_search_id, v_name, v_addr, v_phone, v_site, v_cat, v_rating,
         v_reviews, v_lat, v_lng, v_place, v_cid, v_email, v_ig, v_fb, v_in, v_tw, v_yt)
  ...
  return json_build_object(
    'search_id', v_search_id, 'inserted', v_inserted, 'merged', v_merged,
    'skipped', v_skipped, 'ignored', v_ignored, 'rejected', v_rejected,
    'total', v_total, 'source', v_source, 'replayed', false
  );
end $$;
...
set search_path = public
as $$
  select public.import_leads_v2(
    p_keyword, p_city, p_leads, 'extension', null, null, '{}'::jsonb, 'merge'
  );
$$;

-- Pagina uma importacao pela relacao N:N, mantendo o shape plano de results.
...
as $$
  with filtered as (
    ...
    join public.results r on r.id = sr.result_id and r.user_id = sr.user_id
    where sr.user_id = auth.uid()
      and sr.search_id = p_search_id
      ...
      and (coalesce(p_filters->>'no_social','') <> '1' or (
        coalesce(r.instagram,'') = '' and coalesce(r.facebook,'') = '' and
        coalesce(r.linkedin,'') = '' and coalesce(r.twitter,'') = '' and coalesce(r.youtube,'') = ''
      ))
      and (
        coalesce(p_filters->>'min_rating','') !~ '^[0-9]+(\.[0-9]+)?$'
        or r.rating >= (p_filters->>'min_rating')::real
      )
      and (
        coalesce(p_filters->>'max_rating','') !~ '^[0-9]+(\.[0-9]+)?$'
        or r.rating <= (p_filters->>'max_rating')::real
      )
      and (
        coalesce(p_filters->>'min_reviews','') !~ '^[0-9]+$'
        or r.reviews_count >= (p_filters->>'min_reviews')::int
      )
      and (
        coalesce(p_filters->>'max_reviews','') !~ '^[0-9]+$'
        or r.reviews_count <= (p_filters->>'max_reviews')::int
      )
  ), page_rows as (
  ...
  select json_build_object(
    'data', coalesce((select json_agg(p) from page_rows p), '[]'::json),
    'total', case when p_with_count then (select count(*)::int from filtered) else null end
  );
$$;

-- Exclusao compativel com proveniencia N:N.
...
begin
  ...
  select array_agg(distinct sr.search_id) into v_search_ids
  from public.search_results sr
  ...
  from public.results
  where id = any(coalesce(p_ids, array[]::uuid[])) and user_id = v_uid;
  ...
  update public.searches s
  set total_results = (
    select count(*)::int from public.search_results sr where sr.search_id = s.id
  )
...
begin
  if not exists (
    select 1 from public.searches where id = p_search_id and user_id = v_uid
  ) then
  ...
  -- Reaponta o atalho legado antes de remover a busca.
  ...
  where r.user_id = v_uid
    and coalesce(r.prospect_status,'novo') = 'novo'
    and exists (
      select 1 from public.search_results target
      where target.search_id = p_search_id and target.result_id = r.id and target.user_id = v_uid
    )
    and not exists (
      select 1 from public.search_results other
      where other.result_id = r.id and other.user_id = v_uid and other.search_id <> p_search_id
    );
  get diagnostics v_deleted = row_count;
...
-- Historico de captacao por data de observacao, em vez de mover o lead para a
-- ultima busca ou contar apenas a data de criacao do cadastro canonico.
...
as $$
  select json_build_object(
    ...
    'searches', coalesce((select json_agg(s) from (
      select * from public.searches where user_id = auth.uid() order by created_at desc
    ) s), '[]'::json)
  );
$$;

-- ============================================================================
-- CRM: atividades, status e tarefas
-- ============================================================================
...
begin
  ...
  values
    (v_uid,p_result_id,coalesce(nullif(p_type,''),'note'),coalesce(p_summary,''),
     nullif(p_channel,''),coalesce(p_metadata,'{}'::jsonb),coalesce(p_occurred_at,now()),
...
begin
  ...
  foreach v_id in array coalesce(p_result_ids,array[]::uuid[]) loop
    v_event_key := case when nullif(p_idempotency_key,'') is null then null
                        else p_idempotency_key || ':' || v_id::text end;
    if v_event_key is not null and exists (
      select 1 from public.lead_activities
      where user_id = v_uid and idempotency_key = v_event_key
    ) then
      continue;
    end if;

    select prospect_status into v_from
    from public.results where id = v_id and user_id = v_uid for update;
    ...
    values
      (v_uid,v_id,'status_changed','Status alterado de ' || coalesce(v_from,'novo') || ' para ' || p_status,
...
-- ============================================================================
-- CAMPANHAS TRANSACIONAIS E METRICAS POR TIMESTAMP
-- ============================================================================
...
begin
  ...
  if nullif(p_idempotency_key,'') is not null then
    ...
    where user_id = v_uid and idempotency_key = p_idempotency_key limit 1;
    if found then
      return to_jsonb(v_campaign) || jsonb_build_object('replayed',true);
    end if;
  end if;

  if p_template_id is not null and not exists (
    select 1 from public.campaign_templates where id = p_template_id and user_id = v_uid
  ) then raise exception 'Modelo de campanha nao encontrado'; end if;

  select array_agg(id order by created_at desc), count(*)::int
  into v_ids, v_total
  from (
  ...
  values
    (v_uid,p_template_id,trim(p_name),p_message_body,coalesce(p_filters,'{}'::jsonb),
...
declare
  v_uid uuid := auth.uid();
  v_row public.campaign_leads%rowtype;
  v_from text;
...
begin
  ...
  if nullif(p_idempotency_key,'') is not null and exists (
    ...
    where id = p_campaign_lead_id and user_id = v_uid;
    return v_row;
  end if;
  ...
  v_result_status := case p_status
    when 'sent' then 'enviado' when 'responded' then 'respondeu'
    when 'won' then 'fechado' when 'lost' then 'descartado' else null end;
  if v_result_status is not null then
    update public.results set
      prospect_status = v_result_status,
  ...
  insert into public.lead_activities
    (user_id,result_id,campaign_id,campaign_lead_id,type,summary,channel,
     from_status,to_status,metadata,occurred_at,idempotency_key)
  values
    (v_uid,v_row.result_id,v_row.campaign_id,v_row.id,'campaign_status',
     'Campanha alterada de ' || coalesce(v_from,'pending') || ' para ' || p_status,
     'whatsapp',v_from,p_status,jsonb_build_object('campaign_id',v_row.campaign_id),
     coalesce(p_event_at,now()),nullif(p_idempotency_key,''))
  on conflict (user_id,idempotency_key) where idempotency_key is not null and idempotency_key <> '' do nothing;
  return v_row;
end $$;
...
as $$
  with stats as (
    select campaign_id,
      ...
      count(*) filter (
        where status = 'sent' and followup_due_at is not null and followup_due_at <= now()
  ...
  select coalesce(json_agg(json_build_object(
    ...
    'response_rate',case when coalesce(s.sent_total,0) = 0 then 0
      else round((100.0 * s.responded_total / s.sent_total)::numeric,1) end,
    'conversion_rate',case when coalesce(s.sent_total,0) = 0 then 0
      else round((100.0 * s.won_total / s.sent_total)::numeric,1) end,
    'due_followups',coalesce(s.due_followups,0),
    'first_sent_at',s.first_sent_at,'last_activity_at',s.last_activity_at
...
declare
  v_uid uuid := auth.uid();
  v_job public.mining_jobs%rowtype;
begin
  if v_uid is null then raise exception 'Nao autenticado'; end if;
  ...
  values
    (v_uid,p_client_job_id,nullif(p_keyword,''),nullif(p_city,''),
...
create or replace function public.update_mining_job(
  ...
  p_captured_count int default null,
  p_enriched_count int default null,
  p_inserted_count int default null,
  p_merged_count int default null,
  p_ignored_count int default null,
...
begin
  ...
  if p_search_id is not null and not exists (
    select 1 from public.searches where id = p_search_id and user_id = v_uid
  ) then raise exception 'Busca nao encontrada'; end if;
